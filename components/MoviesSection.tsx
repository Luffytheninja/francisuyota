...
export default function MoviesSection({ movies, onSelectProject }: MoviesSectionProps) {
  return (
    <section id="movies" className="w-full bg-[#3FA360] text-white pt-10 pb-8 px-4 sm:px-8 lg:px-12 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {movies.slice(0, 2).map((movie, index) => (
            <MovieCard
              ...
              project={movie}
              index={index}
              onSelect={() => onSelectProject(movie)}
            />
          ))}
        </div>
      </div>
...
function MovieCard({ project, index, onSelect }: { project: Project; index: number; onSelect: () => void }) {
